...
@section('content')
    <article class="mx-auto max-w-4xl px-6 pb-20 pt-28">
        ...
        @if($article->cover_image_path)
            <img src="{{ asset('storage/'.$article->cover_image_path) }}" alt="{{ $article->title }}" class="mt-8 h-auto w-full rounded-[2rem] border border-slate-200 object-cover shadow-sm">
        @endif
        ...
        @if($article->document_path)
            <div class="mt-10">
                <a href="{{ route('documents.download', ['type' => 'article', 'id' => $article->id]) }}" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-bold text-white">
            ...
            <section class="mt-16 border-t border-slate-200 pt-10">
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-3">
                    @foreach($relatedArticles as $relatedArticle)
                        <a href="{{ route('articles.show', $relatedArticle->slug) }}" class="rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-700 transition hover:border-emerald-300 hover:text-emerald-600">
